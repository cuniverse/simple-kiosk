...
HBITMAP CreateStatusBitmap(bool connected) {
  ...
  const double center = (kSize - 1) / 2.0;
  constexpr double radius = 5.5;
  const std::uint8_t red = connected ? 34 : 148;
  const std::uint8_t green = connected ? 197 : 163;
  const std::uint8_t blue = connected ? 94 : 184;
  for (int y = 0; y < kSize; ++y) {
    for (int x = 0; x < kSize; ++x) {
      ...
      const auto alpha = static_cast<std::uint8_t>(coverage * 255.0);
      const auto premultiplied_red =
          static_cast<std::uint8_t>(red * coverage);
      const auto premultiplied_green =
          static_cast<std::uint8_t>(green * coverage);
      const auto premultiplied_blue =
          static_cast<std::uint8_t>(blue * coverage);
      pixels[y * kSize + x] =
...
void TrayManagerPlugin::_CreateMenu(HMENU menu, flutter::EncodableMap args) {
  ...
  for (flutter::EncodableValue item_value : items) {
    ...
    if (type.compare("separator") == 0) {
      ...
      if (AppendMenuW(menu, uFlags, item_id,
                      g_converter.from_bytes(label).c_str()) != FALSE &&
          icon != nullptr &&
          (icon->compare("status-connected") == 0 ||
           icon->compare("status-disconnected") == 0)) {
        HBITMAP bitmap =
            CreateStatusBitmap(icon->compare("status-connected") == 0);
        if (bitmap != nullptr) {
          ...
          if (SetMenuItemInfoW(menu, position, TRUE, &menu_item_info) != FALSE) {
...
void TrayManagerPlugin::SetContextMenu(
  ...
  std::vector<HBITMAP> previous_bitmaps = std::move(menu_bitmaps);
  menu_bitmaps.clear();
  _CreateMenu(hMenu, std::get<flutter::EncodableMap>(
                         args.at(flutter::EncodableValue("menu"))));
  for (HBITMAP bitmap : previous_bitmaps) {
    DeleteObject(bitmap);
  }
