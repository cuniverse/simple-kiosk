...
class SSH_Message_Channel_Open implements SSHMessage {
  ...
  /// Incoming connection for a previously requested remote Unix socket
  /// forwarding. See OpenSSH PROTOCOL, section 2.4.
  factory SSH_Message_Channel_Open.forwardedStreamLocal({
    required int senderChannel,
    required int initialWindowSize,
    required int maximumPacketSize,
    required String socketPath,
  ...
  factory SSH_Message_Channel_Open.decode(Uint8List bytes) {
    ...
    switch (channelType) {
      ...
      case 'forwarded-streamlocal@openssh.com':
        final socketPath = reader.readUtf8();
        reader.readUtf8(); // reserved
        return SSH_Message_Channel_Open.forwardedStreamLocal(
          senderChannel: senderChannel,
          initialWindowSize: initialWindowSize,
          maximumPacketSize: maximumPacketSize,
          socketPath: socketPath,
