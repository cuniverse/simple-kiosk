...
class SSHClient {
  ...
  final _remoteUnixForwards = <SSHRemoteUnixForward>{};
  ...
  /// Request connections to a Unix domain socket on the remote side be
  /// forwarded to the local side. This is equivalent to
  /// `ssh -R remoteSocketPath:localHost:localPort`.
  Future<SSHRemoteUnixForward?> forwardRemoteUnix(
    String remoteSocketPath,
  ) async {
    await _authenticated.future;

    final reply = await _sendGlobalRequest(
      SSH_Message_Global_Request.streamLocalForward(remoteSocketPath),
    );
    if (reply is SSH_Message_Request_Failure) return null;
    if (reply is! SSH_Message_Request_Success) {
      throw SSHStateError(
        'Unexpected reply to streamlocal-forward request: $reply',
      );
    }

    final forward = SSHRemoteUnixForward(this, remoteSocketPath);
    _remoteUnixForwards.add(forward);
    return forward;
  }

  /// Cancel a previous remote Unix domain socket forwarding.
  Future<bool> cancelForwardRemoteUnix(SSHRemoteUnixForward forward) async {
    await _authenticated.future;
    if (!_remoteUnixForwards.remove(forward)) return false;

    final reply = await _sendGlobalRequest(
      SSH_Message_Global_Request.cancelStreamLocalForward(forward.socketPath),
    );
    return reply is! SSH_Message_Request_Failure;
  }
  ...
  void _handleChannelOpen(Uint8List payload) {
    ...
    switch (message.channelType) {
      ...
      case 'forwarded-streamlocal@openssh.com':
        return _handleForwardedStreamLocalChannelOpen(message);
      case 'x11':
  ...
  void _handleForwardedStreamLocalChannelOpen(
    SSH_Message_Channel_Open message,
  ) {
    final remoteForward = _findRemoteUnixForward(message.socketPath!);
    if (remoteForward == null) {
      final reply = SSH_Message_Channel_Open_Failure(
        recipientChannel: message.senderChannel,
        reasonCode: SSH_Message_Channel_Open_Failure.codeUnknownChannelType,
        description: 'unknown remote Unix forward: $message',
      );
      _sendMessage(reply);
      return;
    }
  ...
  SSHRemoteUnixForward? _findRemoteUnixForward(String socketPath) {
    final result = _remoteUnixForwards.where(
      (forward) => forward.socketPath == socketPath,
    );
    return result.isEmpty ? null : result.first;
  }
...
class SSHRemoteUnixForward {
  final String socketPath;

  SSHRemoteUnixForward(this._client, this.socketPath);
  ...
  void close() {
    _connections.close();
    _client.cancelForwardRemoteUnix(this);
  }

  @override
  String toString() => '$runtimeType($socketPath)';
}
