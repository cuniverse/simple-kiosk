...
- 실행 상태와 버전 확인, 사이니지 표시·감추기, 앱 재시작·종료와 확인 후 Windows PC 종료
- 사이니지 모니터의 실제 데스크톱을 확인하는 저부하 미리보기(기본 2fps·1280px·JPEG 45, FPS·해상도·화질 설정 및 숨김 상태 표시)
- 미리보기의 클릭·드래그를 실제 사이니지에 터치·스와이프로 전달하고 마우스 휠, 가로 휠과 Shift+휠로 스크롤
- 기본 미리보기 크기는 유지하며 **크게 보기**를 누를 때만 미리보기를 브라우저 창에 맞춰 확대. **작게 보기** 또는 **Esc**로 복귀
...
표시합니다. 탭을 벗어나거나 브라우저가 백그라운드로
가면 중지됩니다. 화면 정보도 PIN 인증 뒤 전송되므로 관리
페이지는 운영망의 접근 정책과 방화벽으로 보호하세요.

**사이니지 보이기**는 숨김·최소화 상태를 복원하고 다른 창 뒤의 사이니지를 앞으로 가져와
포커스를 줍니다. **사이니지 앱 종료**는 앱만 종료하며, **PC 종료**는 확인 후 5초 뒤 Windows
종료를 시작합니다. PC가 꺼지면 원격 접속도 끊어집니다.
...
배포본에서는 현재 사용자 로그인 직후 고정 런처를 실행하는 작업 스케줄러 방식으로 등록합니다.
기존 활성 등록은 자동 업데이트 후 첫 실행에서 전환하며, 작업 등록에 성공한 뒤 이전
시작프로그램 바로가기를 정리합니다. 숨김 모드와 자동 실행 해제 상태는 유지합니다.
**등록 삭제**는 예약 작업과 기존 바로가기를 함께 제거합니다. 로그인 전 부팅 시간이나
다른 모든 프로그램보다 먼저 실행되는 순서를 보장하는 기능은 아닙니다.
