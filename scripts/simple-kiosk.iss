...
begin
  DeleteUserData :=
    SuppressibleMsgBox(
      ...
      '예: config, media, exdata, themes, fonts, state, logs, diagnostics, backups 삭제' + #13#10 +
...
begin
  ...
  begin
    ...
    DelTree(AddBackslash(InstallRoot) + 'exdata', True, True, True);
    DelTree(AddBackslash(InstallRoot) + 'fonts', True, True, True);
    DelTree(AddBackslash(InstallRoot) + 'themes', True, True, True);
