...
자동 업데이트에서는 다운로드한 Setup 실행 파일을 직접 열지 않습니다. 단, 사용자가
직접 요청한 업데이트에서 기본 Updater가 실패하면 사용자 승인 후 검증된 Setup으로
전환할 수 있습니다.
...
installer는 자동 업데이트용 런처와 버전 포인터를 자동 구성합니다. **Windows 로그인 직후
...
자동 실행은 현재 사용자 로그인 직후 실행하는 작업 스케줄러 방식으로 등록하며, 등록이
성공한 뒤 기존 시작프로그램 바로가기를 제거합니다. 자동 실행 등록·전환은 동봉된
`updater\configure-startup.ps1`로 처리합니다. 앱 실행은 네이티브 런처가 담당하며,
자동 업데이트의 ZIP 검증·설치·재시작·정상 실행 확인과 실패 시 이전 버전 복구는
네이티브 `ysignage_updater.exe`가 처리합니다.
...
관리자 승인 후 삭제합니다. 무인 설치에서는 설치 중 승인을 요청하지 않으며, 포터블 ZIP과
함께 앱을 처음 실행할 때 규칙이 없으면 그때 관리자 승인을 요청합니다.
...
트레이 메뉴에서 사이니지 보이기·감추기, 사이니지 재시작, WEB 관리자, 설정, 사용자 매뉴얼을 열 수 있습니다.
리버스 포워딩 상태는 `연결됨` 또는 `연결 안 됨`으로 표시됩니다. 실제 포워딩이 확인된
연결 상태에는 녹색 점등 아이콘과 외부 주소가 함께 나타나며, 그 외 상태에는 회색 소등
아이콘이 표시됩니다. 프로그램을 완전히 끝낼 때는 설정 화면 하단 또는 트레이 메뉴의
**완전 종료**를 사용합니다.

### 삭제

Windows 설정의 **앱 > 설치된 앱 > 여의도성당Signage > 제거** 또는 시작 메뉴의 제거 항목을
사용합니다. 제거하면 해당 설치의 자동 실행 예약 작업, 시작 메뉴·바탕화면·시작프로그램 바로가기와 설치된 모든 앱 버전,
업데이트 캐시는 항상 삭제됩니다.
...
이후 **프로그램 폴더의 `ysignage_launcher.exe`**로 앱을 실행하고 **설정 > Windows
시작프로그램**에서 자동 실행을 등록합니다. 설정·로그·업데이트 파일도 기본적으로 이 폴더 아래에
저장됩니다. 별도 위치가 필요할 때만 `SIMPLE_KIOSK_DATA_DIR` 환경변수를 지정합니다.

### Windows 키오스크 잠금

키오스크 키 차단은 기본적으로 꺼져 있습니다(`layout.windowsKioskLockdown: false`).
`true`로 켜면 사이니지 표시 중 `Alt+Tab`, `Alt+Esc`,
...
앱은 시작할 때와 관리 포트·mDNS 설정 저장에 성공했을 때 Setup이 기록한 방화벽 상태를
비교합니다. 규칙이 없거나 설정과 다르면 관리자 승인을 한 번 요청해 기존 관리 규칙을 새
설정으로 교체하고, 관리 API를 끄면 관리 규칙을 제거합니다. 이미 일치하면 PowerShell이나
UAC를 실행하지 않습니다. 승인을 취소해도 앱과 설정 저장은 계속되며 다음 앱 실행 또는
다음 관련 설정 변경 때 다시 시도합니다.
...
화면 보이기·감추기, 업데이트, 앱 재시작·종료와 PC 종료를 수행할 수 있습니다.
**화면 미리보기**는 사이니지가 있는 모니터의 실제 데스크톱을 표시합니다. 기본 미리보기
크기는 유지하며, **크게 보기**를 눌렀을 때만 미리보기를 브라우저 창에 맞춰 확대합니다.
**작게 보기** 또는 **Esc**로 돌아오며, 별도의 보기 배율 선택은 없습니다.
사이니지 영역의 클릭·드래그는 터치·스와이프로 전달하고, 마우스 휠은 가리킨 위치의
스크롤로 전달합니다. 가로 휠과 Shift+휠도 지원합니다. 숨김·최소화 상태, 다른 창이 가린
영역과 오래된 미리보기는 조작할 수 없습니다.
**사이니지 보이기**는 숨김·최소화 상태를 복원하고 다른 창 뒤의 사이니지를 앞으로 가져와
포커스를 줍니다. **사이니지 앱 종료**는 앱만 종료합니다. **PC 종료**는 확인 후 5초 뒤
Windows 종료를 시작하며 PC가 꺼지면 원격 접속도 끊어집니다. 저장하지 않은 작업이 있는
다른 앱은 종료를 막을 수 있고, 종료 권한이 없는 계정에서는 오류가 표시됩니다.
...
| `GET /api/screen-preview` | 모니터 JPEG 미리보기와 프레임 정보 조회 |
| `PUT /api/screen-preview/settings` | 미리보기 FPS·해상도·화질 저장 |
| `POST /api/screen-preview/pointer` | 미리보기 좌표의 터치 누름·이동·해제·취소 |
| `POST /api/screen-preview/wheel` | 미리보기 좌표의 가로·세로 스크롤 전달 |
...
| `POST /api/actions/show` | 사이니지 화면 복원·맨 앞으로 표시·포커스 부여 |
...
| `POST /api/actions/shutdown-pc` | JSON 본문 `{"confirmed":true}` 확인 후 5초 뒤 Windows 종료 시작 |
...
설정의 **지금 업데이트 확인 / 지금 설치**, WEB 관리자 또는 `F9`로 시작한 수동 설치는
같은 버전에서 누적된 실패 횟수와 관계없이 다시 시도합니다. 자동 설치는 같은 대상 버전에서
3회 실패하면 반복 실행을 차단하며, 대상 버전이 바뀌거나 첫 실패 후 24시간이 지나면 실패
횟수와 시간 창을 초기화합니다.

수동 설치에서 네이티브 Updater가 실패하면 종료 코드와 함께 `update-state.json`의 실제
오류를 표시하고 **Setup으로 업데이트**할지 확인합니다. 사용자가 승인한 경우에만 같은
GitHub Release의 `simple-kiosk-windows-setup-<version>.exe`를 다운로드하고 Release
자산의 SHA-256을 검증해 실행합니다. 자동 업데이트에서는 Setup 보조 설치를 실행하지
않습니다.
...
앱 설정의 **진단 정보로 이슈 등록**은 진단 보고서를 먼저 생성한 다음 WEB 관리자의
백업·진단 탭과 GitHub 이슈 작성 영역을 열고 제목 입력란으로 이동합니다. 이슈 등록에는
프로그램 버전과 Windows 환경이 포함되며, GitHub 로그인을 사용하지 않는 운영 환경에서는
GW의 `deploy/github-issue-relay` 중계 서버가 구성되어 있어야 합니다.
...
확인하고 자동 실행을 등록하거나 삭제할 수 있습니다. 등록할 때 다음 실행 방식을 선택합니다.

- **사이니지 모드로 표시**: Windows 로그인 후 전체 화면을 바로 표시합니다. 기본값입니다.
- **숨김 모드로 시작**: 창은 표시하지 않고 트레이에서 실행하며, 트레이 메뉴로 화면을 엽니다.

배포본에서 **시작프로그램 등록** 또는 **등록 정보 저장**을 누르면 현재 사용자 로그인
직후 고정 런처 `ysignage_launcher.exe`를 실행하는 예약 작업을 등록합니다. 작업 등록에
성공한 뒤 기존 시작프로그램 바로가기를 제거하므로 별도의 바로가기를 만들 필요가 없습니다.
포터블 ZIP은 앞의 **ZIP 수동 설치** 절차에 따라 자동 업데이트용 런처를 먼저 구성하세요.

기존 활성 자동 실행 등록은 자동 업데이트 후 첫 실행에서 새 방식으로 전환합니다.
숨김 모드와 자동 실행 해제 상태를 유지하며, 전환에 실패하면 기존 등록을 보존하고 다음
실행에서 다시 시도합니다. 전환 도중 중복 자동 실행 요청이 들어와도 이미 실행 중인 앱을
재시작하지 않습니다. **등록 삭제**를 누르면 예약 작업과 기존 바로가기를 함께 제거합니다.

이 방식은 로그인 뒤의 실행 지연을 줄이며, 로그인 전 부팅 시간이나 다른 모든 프로그램보다
먼저 실행되는 순서를 보장하지는 않습니다.
...
수동 설치는 이전 실패 횟수에 막히지 않습니다. 기본 Updater가 실패하면 오류를 확인한 뒤
Setup 보조 설치를 승인하거나 취소할 수 있습니다. 자동 설치의 3회 실패 차단은 대상 버전
변경 또는 첫 실패 후 24시간 경과 시 완전히 초기화됩니다.
...
1. 프로그램 **설정 > Windows 시작프로그램**에서 **등록 삭제**를 실행한 뒤 앱을 완전히 종료합니다.
2. 직접 등록한 바탕화면 바로가기를 삭제합니다.
