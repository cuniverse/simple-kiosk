...
- 기본 PIN 및 파일 기반 PBKDF2 해시 관리자 인증과 PIN 변경 구현
...
자동 설치는 각 플랫폼 스토어·서명 정책에 맞춘 별도 구현 범위다.

## 1. 목적

운영 중인 사이니지가 GitHub에 배포된 새 버전을 안전하게 확인하고 설치할 수 있게 한다.
업데이트 과정에서 운영자가 변경한 메뉴, URL, 화면 보호기 설정과 사용자 미디어를
보존하면서 새 버전에 추가된 기본 설정은 자동으로 반영해야 한다.

자동 업데이트는 각 사이니지의 관리자 설정에서 켜거나 끌 수 있어야 한다. 설치 실패,
...
- 업데이트 정책 및 설정 폴더는 일반 사이니지 사용자가 수정하지 못하게 ACL을 설정한다.
- 비공개 저장소를 사용할 경우 장기 GitHub 토큰을 사이니지에 평문 저장하지 않는다.
...
- 자동 업데이트는 신규 설치에서도 기본 OFF
- 최초 관리자 PIN은 `1259`, 변경 PIN은 `config/admin-pin.json`의 PBKDF2 해시로 인증
- 기본 설치 가능 시간대는 02:00~05:00, 화면 보호기 진입 후 설치
- 기본 버전 보관 수는 현재/직전 2개, 로그 보관 기간은 30일
- 운영자 추가 메뉴와 제거된 기본 메뉴는 ID 기반 오버라이드 정책 적용
