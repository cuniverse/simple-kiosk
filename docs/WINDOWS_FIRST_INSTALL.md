...
자동 실행을 선택하면 현재 사용자 로그인 직후 `ysignage_launcher.exe`를 실행하는 예약
작업을 등록합니다. 프로그램 **설정 > Windows 시작프로그램**에서 사이니지/숨김 모드를
선택하거나 등록을 삭제할 수 있습니다.
...
기존에 자동 실행을 사용하던 설치는 자동 업데이트 후 첫 실행에서 작업 스케줄러 방식으로
전환합니다. 전환 성공 후 기존 바로가기를 정리하며, 숨김 모드와 자동 실행 해제 상태를
유지합니다. 전환에 실패하면 기존 등록을 유지하고 다음 실행에서 다시 시도합니다.

WEB 관리자 **화면 미리보기**에서 클릭·드래그와 마우스 휠을 실제 사이니지에 전달할 수
있습니다. 기본 크기는 유지하며 **크게 보기**를 누를 때만 확대하고 **작게 보기** 또는
**Esc**로 돌아옵니다. **사이니지 보이기**는 창을 앞으로 가져오고 포커스를 줍니다.
상태 탭의 **사이니지 앱 종료**는 앱만 종료하고, **PC 종료**는 확인 후 5초 뒤 Windows
종료를 시작합니다.
...
- 포터블 실행: 앱 설정에서 시작프로그램 **등록 삭제**를 먼저 실행합니다. 앱을 완전히
  종료하고 직접 만든 바로가기를 제거한 뒤 프로그램 폴더를 삭제합니다.
