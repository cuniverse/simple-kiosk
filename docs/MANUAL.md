...
- **현재 주제 라벨**: `showSelectedTopic` 활성화 시 현재 주제만 읽기 전용으로 표시
...
- Setup의 **사설 네트워크에서 WEB 관리 자동 허용**은 기본으로 선택됩니다. 관리자 승인을
  ...
  않으며 포터블 ZIP에는 자동 적용되지 않습니다.
- Setup 설치 후 관리 API 포트나 mDNS 사용 여부를 바꾸면 설치 폴더의
  `updater/configure-firewall.ps1`을 관리자 PowerShell에서 `-Action Install
  -InstallRoot <설치 폴더>`로 다시 실행해야 방화벽 규칙에도 반영됩니다.
