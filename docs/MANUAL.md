...
- 관리자 페이지에서 상태 확인, 화면 미리보기, 메뉴 설정 변경, 사이니지 보이기·감추기, 업데이트, 앱 재시작·종료와 PC 종료를 수행할 수 있습니다.
- 상태 탭의 **사이니지 앱 종료**는 앱만 종료합니다. **PC 종료**는 확인창을 거쳐 5초 뒤 사이니지가 실행 중인 Windows PC의 종료를 시작합니다. PC가 꺼지면 원격 접속도 끊어집니다. 저장하지 않은 작업이 있는 다른 앱은 종료를 막을 수 있으며, 종료 권한이 없는 Windows 계정에서는 오류가 표시됩니다.
- **화면 미리보기** 탭은 사이니지가 배치된 Windows 모니터의 실제 데스크톱을 운영 확인용 JPEG로 표시합니다.
  ...
  기본 미리보기는 관리자 화면 안의 원래 크기로 표시합니다. **크게 보기**를 누르면 미리보기만 브라우저 창에 맞춰 확대되며, **Esc** 또는 **작게 보기**로 원래 크기로 돌아옵니다. 확대 상태에서도 클릭·드래그·휠 입력 위치를 자동 보정합니다.
  **사이니지 보이기**를 누르면 숨김·최소화 상태를 복원하며, 다른 창 뒤에 있는 사이니지를 앞으로 가져와 키보드 포커스를 줍니다.
  미리보기의 사이니지 영역을 클릭하면 같은 위치에 손가락 터치를 전달합니다.
  ...
  미리보기의 마우스 클릭·드래그도 실제 사이니지에는 손가락 터치로 전달합니다.
  누른 채 움직이면 웹페이지 스크롤·스와이프 등 터치 제스처로 동작하고, 놓으면 종료합니다.
  미리보기 밖에서 놓아도 입력이 해제되며, 탭 전환·포커스 이탈 시 드래그를 취소합니다.
  연결이 끊기면 마지막 입력 후 3초 내에 누름을 자동 해제합니다. 화면 위의 마우스 휠은 해당 사이니지 위치의 세로 스크롤로 전달되며, 가로 휠과 Shift+휠도 지원합니다. 키보드 입력은 지원하지 않습니다.
...
- **Windows**: 사이니지 모드 사용자 계정 + 시작 프로그램으로 앱 자동 실행.
  - 프로그램 **설정 > Windows 시작프로그램**에서 등록 상태와 설치 경로 일치 여부를 확인할 수 있습니다. 배포본에서 **등록 정보 저장** 또는 **시작프로그램 등록**을 누르면 현재 사용자 로그인 시 지연 없이 실행하는 작업 스케줄러 방식으로 등록합니다. 기존 시작프로그램 바로가기는 작업 등록에 성공한 뒤 제거합니다.
  - 기존 설치는 자동 업데이트 뒤 첫 실행에서 기존에 활성화된 자동 실행 등록을 빠른 방식으로 전환합니다. 자동 실행을 등록하지 않았거나 사용 중지한 경우에는 새로 켜지 않습니다. 새 설치의 자동 실행 옵션도 같은 방식을 사용합니다. 사이니지/숨김 모드와 업데이트 후 런처 경로를 유지하며, **등록 삭제** 시 예약 작업과 기존 바로가기를 함께 제거합니다.
  - 반복 업데이트는 사용자별 같은 예약 작업 하나를 사용하며, 기존 바로가기는 작업 등록 성공 후 정리합니다. 자동 시작 요청이 겹쳐도 실행 중인 사이니지를 재시작하지 않습니다. 전환 실패 시 기존 등록을 유지하고 다음 실행에서 재시도하며, 설정의 **등록 정보 저장**으로 직접 전환할 수도 있습니다.
  - 이 기능은 로그인 뒤 자동 실행 지연을 줄입니다. 로그인 전 부팅 시간이나 다른 모든 프로그램보다 먼저 실행되는 순서를 보장하지는 않습니다.
