...
- `F12`: 프로그램·Updater 버전과 제작자 이메일·GitHub 정보 표시
- `F9`: 업데이트 확인 후 사용자 동의 시 다운로드, 설치 및 자동 재시작

### Windows 시스템 트레이

- 프로그램 실행 중 Windows 알림 영역에 **Simple Kiosk** 트레이 아이콘이 표시됩니다.
- 프로그램 메뉴의 **키오스크 감추기** 아이콘을 누르면 화면만 감추고 프로그램은 계속 실행됩니다.
- 창 닫기 또는 `Alt + F4`도 완전히 종료하지 않고 키오스크 화면을 감춥니다.
- 트레이 아이콘을 왼쪽 클릭하면 키오스크 화면을 보이거나 감출 수 있습니다.
- 트레이 메뉴에서 **키오스크 보이기**, **키오스크 감추기**, **설정**, **사용자 매뉴얼**, **완전 종료**를 선택할 수 있습니다.
...
- 다른 PC에서 `http://<키오스크 IP>:<포트>/`에 접속하면 관리자 페이지가 열립니다.
- 로그인 PIN은 프로그램 설정에 사용하는 관리자 PIN과 같습니다. PIN 파일을 삭제한 상태의 기본값은 `1259`입니다.
- 관리자 페이지에서 상태 확인, 메뉴 JSON 변경, 키오스크 보이기·감추기, 업데이트, 재시작과 완전 종료를 수행할 수 있습니다.
- 메뉴 JSON은 저장 전에 기본 설정과 병합 검증되며, 올바른 설정은 저장 직후 키오스크에 적용됩니다.
